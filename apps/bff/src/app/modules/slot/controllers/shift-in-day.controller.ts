...
export class ShiftInDayController {
    ...
    async getAll(@ProcessId() processId: string) {
        console.log('con chó')
        const rs = await firstValueFrom(this.shiftInDayClient.send<ShiftInDayTcpResponse[], string>(TCP_SLOT_SERVICE_MESSAGE.GET_SHIFT_IN_DAY, { data: null, processId }).pipe(map(row => row.data)));
